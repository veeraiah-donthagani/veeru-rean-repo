...
user node['REAN-jenkins']['master']['user'] do
  ...
  system node['REAN-jenkins']['master']['use_system_accounts'] # ~FC048
end
...
remote_file File.join(node['REAN-jenkins']['master']['home'], 'jenkins.war') do
  source   node['REAN-jenkins']['master']['source']
  checksum node['REAN-jenkins']['master']['checksum'] if node['jenkins']['master']['checksum']
  owner    node['REAN-jenkins']['master']['user']
  group    node['REAN-jenkins']['master']['group']
...
runit_service 'jenkins' do
  sv_timeout node['REAN-jenkins']['master']['runit']['sv_timeout']
end
